from htmlnode import *
from textnode import *
import re


def split_nodes_delimiter(old_nodes, delimiter, text_type):
    new_nodes = []
    for node in old_nodes:
        if node.text.count(delimiter) % 2 != 0:
            raise Exception("Unclosed Syntax")
        if node.text_type != TextType.TEXT:
            new_nodes.append(node)
        else:
            splittext = node.text.split(delimiter)
            for index in range(0, len(splittext)):
                if splittext[index] == "":
                    pass
                else:
                    if index % 2 == 0:
                        new_nodes.append(TextNode(splittext[index], TextType.TEXT))
                    else:
                        new_nodes.append(TextNode(splittext[index], text_type))
    return new_nodes

def split_nodes_image(old_nodes):
    new_nodes = []
    for node in old_nodes:
        if node.text_type != TextType.TEXT:
            new_nodes.append(node)
            continue
        text = node.text
        extracted_tup = extract_markdown_images(text)
        for image_alt, image_link in extracted_tup:
            splittext = text.split(f"![{image_alt}]({image_link})", 1)
            if splittext[0] != "":
                new_nodes.append(TextNode(splittext[0], TextType.TEXT))
            new_nodes.append(TextNode(image_alt, TextType.IMAGE, image_link))
            text = splittext[1]
        if text != "":
            new_nodes.append(TextNode(text, TextType.TEXT))
    return new_nodes


def split_nodes_link(old_nodes):
    new_nodes = []
    for node in old_nodes:
        if node.text_type != TextType.TEXT:
            new_nodes.append(node)
            continue
        text = node.text
        extracted_tup = extract_markdown_links(text)
        for link_text, link_link in extracted_tup:
            splittext = text.split(f"[{link_text}]({link_link})", 1)
            if splittext[0] != "":
                new_nodes.append(TextNode(splittext[0], TextType.TEXT))
            new_nodes.append(TextNode(link_text, TextType.LINK, link_link))
            text = splittext[1]
        if text != "":
            new_nodes.append(TextNode(text, TextType.TEXT))
    return new_nodes                    


def extract_markdown_images(text):
    matches = re.findall(r"!\[([^\[\]]*)\]\(([^\(\)]*)\)", text)
    return matches

def extract_markdown_links(text):
    matches = re.findall(r"(?<!!)\[([^\[\]]*)\]\(([^\(\)]*)\)", text)
    return matches


def text_to_textnodes(text):
    original = text.copy()
    final = []
    code_text = split_nodes_delimiter(original, "`", TextType.CODE)
    bold_text = split_nodes_delimiter(code_text, "**", TextType.BOLD)
    italic_text = split_nodes_delimiter(bold_text, "_", TextType.ITALIC)
    images = split_nodes_image(italic_text)
    links = split_nodes_link(images)
    final.extend(links)
    return final